  ...
  for (let y= 0 ; y<noi_height;y++) {
    for ( let x = 0 ; x <noi_width; x ++) {
     ...
     var trans_y = posY+(margin+size)*y;
      ...
      push();
      myDraw[index] = new MimiDraw(index,x,y,size,random(0,360),random(0.1,3.9),'#FFC286',bool,trans_x,trans_y,disappear);
...
class MimiDraw {
  constructor(index,arr_x,arr_y,size,rotation,weight,color,side,trans_x,trans_y,dis){
    this.index = index;
    this.arr_x=arr_x;
    this.arr_y=arr_y;
    this.size = size; 
